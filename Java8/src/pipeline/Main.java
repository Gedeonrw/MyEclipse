...
import Quiz3.quizclasses.Employee;
import Quiz3.quizclasses.EmployeeTestData;
import Quiz3.quizclasses.Pair;
import Quiz3.quizclasses.Trader;
import Quiz3.quizclasses.TraderTransactTestData;
import Quiz3.quizclasses.Transaction;
...
public class Main {
	...
	List<Pair> pairList = list.stream()
			            .filter(e -> e.getSalary() >= 60000 && e.getSalary() < 130000 )
			            .map(e -> new Pair(e.getName(), e.getSalary()))
			            .sorted(Comparator.comparing((Pair p) -> p.name).thenComparing(p -> p.salary))
			            .collect(Collectors.toList());
	...
	public static void prob2() {
		...
		List<Transaction> lstTransaction = list.stream()
	...
	public static void prob3() {
		//Use this list
		List<Transaction> list = TraderTransactTestData.getTransactions();  
		List<Trader> trd = list.stream()
				    .map(tr -> tr.getTrader())
				    .distinct()
				    .filter(e -> e.getCity().equals("cambridge"))
				    .sorted(Comparator.comparing(c -> c.getName())) // Trader::getName
				    .collect(Collectors.toList());
