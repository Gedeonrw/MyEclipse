...
public class Main {
	...
	public static void prob2() {
		...
		List<Transaction> tr = list.stream().filter(e -> e.getYear()>=2011).sorted(Comparator.comparing(Transaction::getValue))
                .collect(Collectors.toList());
	...
	public static void prob3() {
		...
		List<Transaction> traders = list.stream().filter(e -> e.getTrader().getCity().equals("Cambridge"))
                .sorted((p1, p2) -> p1.getTrader().getName().compareTo(p2.getTrader().getName()))
                .collect(Collectors.toList());
