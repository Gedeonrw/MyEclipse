...
//				.sorted(Comparator.comparing(Pair::name)
//						.thenComparing(Pair::getsalary , Comparator.reverseOrder())
//				.collect(Collectors.toList());
//				        
	...
	public static void prob2() {
		...
		List<Transaction> tr = list.stream().filter(e -> e.getYear()==2011)
	...
	public static void prob3() {
		...
		List<Transaction> tradrs = list.stream()
				.filter(e -> e.getTrader().getCity().equals("Cambridge"))
                .sorted((t1, t2) -> t1.getTrader().getName().compareTo(t2.getTrader().getName()))
                .collect(Collectors.toList());
		
                 System.out.println(tradrs);   
                 
                 //PRACTICE IT WITH USING SET
